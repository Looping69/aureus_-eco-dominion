...
export type SubsurfaceExcavationOptions = { deformSurface?: boolean };

export function layeredChunkKey(x: number, z: number): string { return `${Math.floor(x / SUBSURFACE_CHUNK_SIZE)},${Math.floor(z / SUBSURFACE_CHUNK_SIZE)}`; }
export function layeredCellKey(x: number, y: number, z: number): string { return `${x},${y},${z}`; }
export function subsurfaceDigJobId(x: number, y: number, z: number): string { return `${SUBSURFACE_DIG_JOB_PREFIX}_${x}_${y}_${z}`; }
export function subsurfaceClearRubbleJobId(x: number, y: number, z: number): string { return `${SUBSURFACE_CLEAR_RUBBLE_JOB_PREFIX}_${x}_${y}_${z}`; }
export function isSubsurfaceDigJob(job: { id?: string; targetY?: number }): boolean { return Boolean(job.id?.startsWith(`${SUBSURFACE_DIG_JOB_PREFIX}_`) || job.id?.startsWith(`${SUBSURFACE_CLEAR_RUBBLE_JOB_PREFIX}_`) || Number.isFinite(job.targetY)); }
export function isSubsurfaceLayer(layeredWorld: LayeredWorldState, y: number): boolean { return y < layeredWorld.surfaceY && y >= layeredWorld.minY && y <= layeredWorld.maxY; }
export function getOpenPitEntryLayer(layeredWorld: LayeredWorldState): number { return Math.max(layeredWorld.minY, layeredWorld.surfaceY - SUBSURFACE_OPEN_PIT_ENTRY_DEPTH); }
export function getSubsurfaceCell(layeredWorld: LayeredWorldState, x: number, y: number, z: number): WorldVoxelCell | undefined { return layeredWorld.chunks?.[layeredChunkKey(x, z)]?.layers?.[y]?.cells?.[layeredCellKey(x, y, z)]; }
export function setActiveSubsurfaceLayer(layeredWorld: LayeredWorldState, y: number): LayeredWorldState { return { ...layeredWorld, activeY: Math.max(layeredWorld.minY, Math.min(layeredWorld.maxY, Math.round(y))), renderVersion: (layeredWorld.renderVersion || 0) + 1 }; }

function getRubbleDropZoneKey(x: number, y: number, z: number): string { return `${x},${y},${z}`; }
function ensureRubbleState(layeredWorld: LayeredWorldState): void { layeredWorld.rubbleStockpile ??= 0; layeredWorld.rubbleDropZones ??= {}; }
...
function getTotalRubbleCapacity(state: GameState): number {
    ...
    const subsurfaceCapacity = Object.values(state.layeredWorld.rubbleDropZones || {}).reduce((total, zone) => total + zone.capacity, 0);
...
function ensureOpenPitMetrics(tile: GridTile): { baseHeight: number; depth: number } {
    const depth = Math.max(0, Math.min(SUBSURFACE_MAX_OPEN_PIT_DEPTH, Math.round(tile.openPitDepth ?? 0)));
    const baseHeight = tile.openPitBaseHeight ?? tile.terrainHeight + (depth * SUBSURFACE_TERRAIN_DROP_PER_LAYER);
    tile.openPitBaseHeight = baseHeight;
    tile.openPitDepth = depth;
...
export function validateSubsurfaceDigTarget(state: GameState, x: number, y: number, z: number): CommandResult {
    ...
    if ((tile.openPitDepth ?? 0) >= SUBSURFACE_MAX_OPEN_PIT_DEPTH && y <= layeredWorld.surfaceY - SUBSURFACE_MAX_OPEN_PIT_DEPTH) return { ok: false, code: CommandErrorCode.INVALID_TARGET, reason: 'This open pit is already at the 2-level safety limit.' };
...
export function validateSubsurfaceRubbleClearTarget(state: GameState, x: number, y: number, z: number): CommandResult {
    ...
    if (!hasRubbleDropCapacity(state)) return { ok: false, code: CommandErrorCode.INVALID_TARGET, reason: 'Build a Stockpile or designate a rubble dump with free space before clearing this pile.' };
    return { ok: true };
}
...
export function queueSubsurfaceExcavationJob(state: GameState, x: number, y: number, z: number): CommandResult {
    ...
    if (existing) { existing.priority = Math.max(existing.priority, 88); return { ok: true }; }
...
export function queueSubsurfaceRubbleClearJob(state: GameState, x: number, y: number, z: number): CommandResult {
    ...
    const existing = state.jobs.find(job => job.id === id);
    if (existing) { existing.priority = Math.max(existing.priority, 86); return { ok: true }; }
...
function refreshSurfaceTiles(state: GameState, coords: Array<{ x: number; z: number }>): void {
    ...
    for (const { cx, cz, updates } of updatesByChunk.values()) state.pendingEffects.push({ type: 'CHUNK_UPDATE', cx, cz, updates });
}

function refreshSurfaceTile(state: GameState, x: number, z: number): void { refreshSurfaceTiles(state, [{ x, z }]); }
function isOpenPitSurfaceTile(state: GameState, x: number, z: number): boolean { const tile = ChunkStore.getTile(state.chunks, x, z); return Boolean(tile && tile.buildingType === BuildingType.EMPTY && (tile.foliage === 'ROCK_PEBBLE' || (tile.openPitDepth ?? 0) > 0)); }
...
function markSurfaceRubble(state: GameState, x: number, z: number): void { const tile = ChunkStore.getTile(state.chunks, x, z); if (!tile || tile.buildingType !== BuildingType.EMPTY) return; tile.foliage = 'ROCK_PEBBLE'; tile.markedForHarvest = false; syncConnectedOpenPit(state, x, z); }
function clearSurfaceRubble(state: GameState, x: number, z: number): void { const tile = ChunkStore.getTile(state.chunks, x, z); if (!tile || tile.foliage !== 'ROCK_PEBBLE') return; tile.foliage = 'NONE'; refreshSurfaceTile(state, x, z); }
function markSubsurfaceDirty(layeredWorld: LayeredWorldState, x: number, y: number, z: number): void { const chunk = layeredWorld.chunks[layeredChunkKey(x, z)]; const layer = chunk?.layers?.[y]; if (layer) layer.dirty = true; if (chunk) chunk.dirty = true; layeredWorld.renderVersion = (layeredWorld.renderVersion || 0) + 1; }
...
function breakSubsurfaceCellIntoRubble(state: GameState, cell: WorldVoxelCell, options: SubsurfaceExcavationOptions): CommandResult {
    ...
    if (options.deformSurface) { lowerSurfaceForOpenPit(state, cell.x, cell.z); markSurfaceRubble(state, cell.x, cell.z); }
    return { ok: true };
}
...
function clearRubbleCell(state: GameState, cell: WorldVoxelCell): CommandResult {
    ...
    if (!validation.ok) return validation;
    if (!depositRubble(state, SUBSURFACE_RUBBLE_PER_BLOCK)) return { ok: false, code: CommandErrorCode.INVALID_TARGET, reason: 'Build a Stockpile or designate a rubble dump with free space before clearing this pile.' };
